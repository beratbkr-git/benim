...
class DatatableExtend {
  ...
  _addListeners() {
    ...
    if (document.getElementById("datatableCheckAllButton")) {
      document
        ...
        .addEventListener("click", this._onCheckAllButtonClick.bind(this));
    }
  ...
  _onRowClick(event) {
    ...
    // NEW LOGIC: If the clicked element or any of its ancestors is an <a> tag,
    // let the default link behavior happen and do not toggle the checkbox.
    if (event.target.tagName === "A" || event.target.closest("a")) {
      return;
    }
    // Eğer tıklanan öğe bir <div> ise, hiçbir şey yapma
    if (event.target.tagName === "DIV" || event.target.closest("div")) {
      return;
    }
    if (
    ...
    if (event.target.classList.contains("etkisiz")) {
      return true;
    }
    // Eğer tıklanan öğe bir dropdown butonu veya içeriğiyse, checkbox her zaman seçili kalmalı
    ...
    event.preventDefault(); // Prevent default only if we are handling the row selection
  ...
  _onExportClick(event) {
    ...
    if (event.currentTarget.classList.contains("export-copy")) {
      try {
        this._getDatatable(event.currentTarget.closest(".datatable-export"))
          .buttons(0)
          .trigger();
      } catch (error) {
        console.log("Trigger button is not found");
