...
class Settings {
  ...
  _mergeAttributesFromStorage() {
    for (const prop in this.settings.attributes) {
      if (localStorage.getItem(this.settings.storagePrefix + prop)) {
        this.settings.attributes[prop] = localStorage.getItem(
          this.settings.storagePrefix + prop
        );
      } else {
        localStorage.setItem(
          this.settings.storagePrefix + prop,
          this.settings.attributes[prop]
        );
      }
    }
  }
  ...
  _mergeOverrides() {
    ...
    const overridedAttributes = Object.assign(
      this.settings.attributes,
      overrideJSON.attributes
    );
  ...
  _setAttributes() {
    for (const prop in this.settings.attributes) {
      document.documentElement.setAttribute(
        this.attributeOptions[prop].attribute,
        this.settings.attributes[prop]
      );
    }
  }
  ...
  updateAttribute(id, value) {
    ...
    if (this.settings.attributes[id] !== value) {
      ...
      if (this.attributeOptions[id].update) {
        document.documentElement.setAttribute(
          this.attributeOptions[id].attribute,
          value
        );
      }
      if (this.attributeOptions[id].event) {
        document.documentElement.dispatchEvent(
          new CustomEvent(this.attributeOptions[id].event, { detail: value })
        );
      }
    }
  }
