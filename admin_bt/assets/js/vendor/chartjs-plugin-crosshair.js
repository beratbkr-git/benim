  ...
  var TracePlugin = function (Chart$$1) {
    ...
    var crosshairPlugin = {
      ...
      doZoom: function (chart, start, end) {
        ...
        // notify delegate
        var beforeZoomCallback = helpers.getValueOrDefault(
        ...
        for (var datasetIndex = 0; datasetIndex < chart.data.datasets.length; datasetIndex++) {
          ...
          for (var oldDataIndex = 0; oldDataIndex < sourceDataset.length; oldDataIndex++) {
            ...
            if (oldData.t >= start && !started && index > 0) {
              newData.push(sourceDataset[index - 1]);
              started = true;
            }
            if (oldData.t >= start && oldData.t <= end) {
              newData.push(oldData);
            }
            if (oldData.t > end && !stop && index < sourceDataset.length) {
              newData.push(oldData);
              stop = true;
            }
            index += 1;
